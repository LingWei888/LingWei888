<h1 align="center">👋 你好，我是 零微</h1>

<p align="center">
  <a href="https://github.com/lingwei888"><img src="https://img.shields.io/github/followers/lingwei888?label=Follow&style=social" alt="GitHub 关注者"></a>
  <a href="mailto:lwei10036@gmail.com"><img src="https://img.shields.io/badge/Email-联系我-blue?style=flat-square&logo=gmail&logoColor=white" alt="邮箱"></a>
</p>

---

### 📍 关于我
- 🏫 **河北科技大学 2028 届本科生**  
- 🔬 码灵程序设计实验室 学生负责人  
- 💼 曾在 **龙即信息** 实习，参与企业级项目开发  
- 🏆 获奖经历：  
  - 计算机设计大赛 国家级三等奖  
  - 蓝桥杯 河北省一等奖  
- 💻 初二开始接触 PHP 开发，对编程拥有超过 6 年的热爱与实践  
- 🎯 专注于 Java 后端 + Vue 前端全栈开发，追求高效简洁的代码风格

---

### 🛠️ 技术栈
<p align="left">
  <!-- 主技术栈 -->
  <img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=java&logoColor=white">
  <img src="https://img.shields.io/badge/Vue-4FC08D?style=for-the-badge&logo=vue.js&logoColor=white">
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white">
  <img src="https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white">
  
  <!-- 辅助技术 -->
  <img src="https://img.shields.io/badge/SpringBoot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white">
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white">
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white">
</p>

---

### 🚀 代表项目
| 项目名称 | 技术栈 | 简介 |
|---------|--------|------|
| **仓库管理系统** | Java + Vue + MySQL | 实现库存实时监控、出入库记录与报表生成，支持多角色权限管理 |
| **人才评估认知计算系统** | SpringBoot + Vue + 机器学习算法 | 基于用户行为数据构建评估模型，辅助企业人才选拔决策 |
| **智能导游系统** | Java + Vue + 高德地图API | 提供景点推荐、路线规划与语音讲解功能，优化旅游体验 |
| **零工在线经济协作平台** | 微服务架构 + Vue | 连接零工与雇主的一站式服务平台，支持订单追踪与支付集成 |
| **码灵鼠的程序天地小程序** | 微信小程序 + Java 后端 | 面向初学者的编程学习工具，包含题库、教程与代码运行环境 |

---

### 📊 GitHub 统计
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=lingwei888&show_icons=true&theme=blue-green&hide_border=true" width="48%" alt="GitHub 统计">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=lingwei888&layout=compact&theme=blue-green&hide_border=true" width="40%" alt="语言统计">
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=lingwei888&theme=blue&hide_border=true&area=true" alt="活动图表">
</p>

---

<p align="center">
  <span>ExitCode-退出传统代码，跨越新时代-极客跃迁🌟</span>
</p>
